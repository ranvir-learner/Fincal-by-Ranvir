...
const CALCULATORS = [
  {
    ...
    icon: <TrendingUp size={24} className="text-[#185FA5]" />,
  },
  {
    id: "lumpsum",
    category: "Investment Calculators",
    name: "Lumpsum Calculator",
    desc: "Calculate returns for lump sum investments to achieve your financial goals",
    icon: <PiggyBank size={24} className="text-[#185FA5]" />,
  },
  {
    id: "swp",
    category: "Investment Calculators",
    name: "SWP Calculator",
    desc: "Calculate your final amount with Systematic Withdrawal Plans",
    icon: <Briefcase size={24} className="text-[#185FA5]" />,
  },
  {
    id: "mf",
    category: "Investment Calculators",
  ...
  },
  {
    ...
    category: "Investment Calculators",
    name: "FD Calculator",
    desc: "Calculate the returns on your fixed deposits",
    icon: <PiggyBank size={24} className="text-[#185FA5]" />,
  },
  {
    id: "ppf",
    category: "Investment Calculators",
    name: "PPF Calculator",
    desc: "Calculate the returns on Public Provident Fund",
    icon: <LandmarkIcon size={24} className="text-[#185FA5]" />,
  },

  {
    id: "fire-target",
    category: "Retirement & Generational Wealth",
    name: "FIRE Calculator",
    desc: "Find your Financial Independence number and how much corpus you need",
    icon: <LandmarkIcon size={24} className="text-[#3B6D11]" />,
  },
  {
    id: "fire-readiness",
    category: "Retirement & Generational Wealth",
    name: "FIRE Readiness",
    desc: "Check if you can safely retire now with your current corpus",
    icon: <TrendingUp size={24} className="text-[#3B6D11]" />,
  },
  {
    id: "fire-date",
    category: "Retirement & Generational Wealth",
    name: "Retirement Date",
    desc: "Find out the exact year and age when you can retire",
    icon: <Search size={24} className="text-[#3B6D11]" />,
  },

  {
    ...
    desc: "Calculate EMI, total interest and amortisation for your home loan",
    icon: <Home size={24} className="text-[#A32D2D]" />,
  },
  {
    id: "car",
    category: "Loan Calculators",
    name: "Car Loan Calculator",
    desc: "Calculate monthly EMI and total cost for your car loan",
    icon: <Car size={24} className="text-[#A32D2D]" />,
  },
  {
    id: "personal",
    category: "Loan Calculators",
    name: "Personal Loan Calculator",
    desc: "Calculate EMI and repayment schedule for your personal loan",
    icon: <Calculator size={24} className="text-[#A32D2D]" />,
  },
  {
    id: "debt-payoff",
    category: "Loan Calculators",
    name: "Debt Payoff Strategy",
    desc: "Compare Snowball and Avalanche repayment methods",
    icon: <Calculator size={24} className="text-[#A32D2D]" />,
  },
];
...
export default function App() {
  ...
  const investments = filtered.filter(
    (c) => c.category === "Investment Calculators",
  );
  const retirement = filtered.filter(
    (c) => c.category === "Retirement & Generational Wealth",
  );
    ...
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-slate-950 text-[#1E293B] dark:text-slate-200 font-sans flex flex-col transition-colors duration-300">
      <header className="sticky top-0 z-50 flex items-center justify-between px-4 md:px-8 py-4 bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 transition-colors duration-300">
        ...
        <div className="flex justify-end items-center gap-2 sm:gap-4 md:ml-auto">
          ...
          <a
            href="https://forms.gle/942NYgRvhxhEykwn7"
            target="_blank"
            rel="noopener noreferrer"
            className="w-10 h-10 shrink-0 flex items-center justify-center rounded-full bg-slate-100 dark:bg-slate-900 border border-transparent dark:border-slate-800 text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 transition-colors"
            title="Give Feedback"
          >
          ...
          <select
            value={symbol}
            onChange={(e) =>
              setCurrencySymbol(e.target.value as CurrencySymbol)
            }
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full p-4 md:p-8 flex flex-col gap-10">
        {!search && recentCalculators.length > 0 && (
          <section>
        ...
        {investments.length > 0 && (
          <section>
            <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-6 flex items-center gap-2">
              <TrendingUp className="text-[#185FA5]" /> Investment Calculators
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {investments.map((calc) => (
                <CalcCard
                  key={calc.id}
                  calc={calc}
                  onClick={() => handleOpenCalc(calc.id)}
        ...
        {retirement.length > 0 && (
          <section>
            <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-6 flex items-center gap-2">
              <LandmarkIcon className="text-[#3B6D11]" /> Retirement &
              Generational Wealth
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {retirement.map((calc) => (
        ...
        {loans.length > 0 && (
          <section>
            <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-6 flex items-center gap-2">
              <Home className="text-[#A32D2D]" /> Loan Calculators
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {loans.map((calc) => (
                <CalcCard
...
function CalcCard({ calc, onClick }: any) {
  return (
    ...
    >
      <div>
        <h3 className="font-bold text-base text-slate-800 dark:text-slate-100 group-hover:text-[#185FA5] dark:group-hover:text-[#4299E1] transition-colors">
          {calc.name}
        </h3>
      ...
      <div className="flex justify-between items-end mt-4">
        <span className="text-xs font-semibold text-[#185FA5] dark:text-[#4299E1] opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
...
function RecentCard({ calc, onClick }: any) {
  return (
    <div
      onClick={onClick}
      className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-[#185FA5]/40 dark:hover:border-[#185FA5]/60 hover:shadow-sm transition-all cursor-pointer group flex items-center gap-2 px-2.5 py-1.5 rounded-lg w-fit"
    >
      <div className="w-5 h-5 rounded flex items-center justify-center shrink-0 text-slate-500 dark:text-slate-400 group-hover:text-[#185FA5] dark:group-hover:text-[#4299E1] transition-colors [&>svg]:w-3.5 [&>svg]:h-3.5">
        {calc.icon}
      </div>
      <span className="font-semibold text-xs text-slate-800 dark:text-slate-100 group-hover:text-[#185FA5] dark:group-hover:text-[#4299E1] transition-colors pr-1">
        {calc.name}
      </span>
    </div>
  );
